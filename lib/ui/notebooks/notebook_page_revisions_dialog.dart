...
/// Lists a notebook page's saved revisions and lets the user save the current
/// state, delete revisions, or restore one. Mirrors [SermonRevisionsDialog]:
/// restoring pops with the chosen revision so the open editor applies it in one
/// place.
class NotebookPageRevisionsDialog extends ConsumerWidget {
  final String pageId;
  final String currentTitle;
  final String currentContent;

  const NotebookPageRevisionsDialog({
    super.key,
  ...
  static Future<NotebookPageRevision?> show(
    BuildContext context, {
    required String pageId,
    required String currentTitle,
    required String currentContent,
  }) {
    return showDialog<NotebookPageRevision?>(
      context: context,
      builder: (_) => NotebookPageRevisionsDialog(
        pageId: pageId,
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    final revisionsAsync = ref.watch(notebookPageRevisionsProvider(pageId));
...
class _RevisionTile extends ConsumerWidget {
  final NotebookPageRevision revision;
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return ListTile(
      leading: Icon(icon),
      title: Text(
        revision.label?.isNotEmpty == true
            ? '${revision.label} · $when'
            : when,
      ),
      subtitle: Column(
