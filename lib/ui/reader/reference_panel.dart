...
/// Browsable reference tables — kings & reigns today, more datasets (as
/// tabs) in later phases: a searchable list grouped by realm, each opening
/// to a short account of the reign with tappable passage links.
class ReferencePanel extends ConsumerStatefulWidget {
  const ReferencePanel({super.key});

  @override
  ConsumerState<ReferencePanel> createState() => _ReferencePanelState();
}

class _ReferencePanelState extends ConsumerState<ReferencePanel> {
...
class _KingReignList extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    for (final realm in Realm.values) {
      ...
      rows.add(_RealmHeader(label: realm.label));
...
class _RealmHeader extends StatelessWidget {
  const _RealmHeader({required this.label});
  ...
  Widget build(BuildContext context) {
    ...
    return ListView(
      ...
      children: [
        ...
        Text(king.notes, style: Theme.of(context).textTheme.bodyMedium),
        if (king.citations.isNotEmpty)
          Padding(
            padding: const EdgeInsets.only(top: 10),
            child: Wrap(
              spacing: 6,
              runSpacing: 6,
              children: king.citations
                  .map((p) => ActionChip(
                        visualDensity: VisualDensity.compact,
