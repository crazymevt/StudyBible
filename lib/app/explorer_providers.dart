...
final explorerEventDetailProvider =
  ...
  if (additionNames != null) {
    final additionRows = await (store.select(store.places)
          ..where((p) => p.name.isIn(additionNames)))
        .get();
...
final explorerPersonStoriesProvider =
  ...
  return [
    for (final r in rows) ExplorerTopicHit(r.read<int>('id'), r.read<String>('name')),
  ];
});
