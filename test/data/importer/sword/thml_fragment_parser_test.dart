...
void main() {
  group('parseThmlFragment', () {
    test('attaches a trailing <sync> Strong\'s to the preceding word', () {
      ...
      expect(
        r.segments.firstWhere((s) => s.text.trim() == 'Jesus').strongs,
        'G2424',
      );
      expect(
        r.segments.firstWhere((s) => s.text.trim() == 'Christ').strongs,
        'G5547',
      );
    });
    ...
    test('captures <note> as a footnote kept out of plain text', () {
      ...
      expect(
        r.segments.singleWhere((s) => s.isFootnote).footnoteText,
        'Heb. eretz',
      );
    });
    ...
    test('marks <i> as italic and breaks on <br/>', () {
      ...
      expect(
        r.segments.firstWhere((s) => s.text.trim() == 'added').isItalic,
        isTrue,
      );
